...
public class ConditionTextSearch : Condition
{
    ...
    public void AddSubquerySearchField<TSubquery>(
        string propertyName,
        string keyColumn,
        string idColumn)
    {
        var subquery =
            new ConditionSubquery<string, TSubquery>(null, propertyName, keyColumn, idColumn)
                { UseLike = true };
    ...
    public void AddSubquerySearchField<TSubquery>(
        ...
        string alias)
    {
        var subquery = new ConditionSubquery<string, TSubquery>(null, propertyName, keyColumn,
            idColumn,
    ...
    public override ICriterion GetCriterion()
    {
        ...
        foreach (var searchTerm in _items)
        {
            ...
            foreach (var column in _propertyNames)
                disjunction.Add(
                    Restrictions.InsensitiveLike(column, searchTerm, MatchMode.Anywhere));
