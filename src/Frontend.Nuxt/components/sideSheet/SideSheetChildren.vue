...
const toggleChild = async (childId: number) => {
    sideSheetStore.toggleExpanded(childId)

    if (sideSheetStore.expandedPages.has(childId) && !sideSheetStore.childrenMap.has(childId)) {
...
<template>
    <div class="sidesheet-children" :style="{ paddingLeft: `${depth * 16}px` }">
        <div v-for="child in children" :key="child.id">
            <div class="content-item">
                <div v-if="child.childrenCount > 0" class="expand-toggle" @click.stop="toggleChild(child.id)">
                    <font-awesome-icon
                        :icon="sideSheetStore.expandedPages.has(child.id) ? ['fas', 'angle-down'] : ['fas', 'angle-right']" />
                </div>
                ...
                <NuxtLink :to="$urlHelper.getPageUrl(child.name, child.id)"
                    ...
                    <div class="link">
                        <img :src="child.imgUrl" class="sidesheet-thumb" />
                        <span class="link-text">{{ child.name }}</span>
                    </div>
                </NuxtLink>
            </div>
            <SideSheetChildren v-if="sideSheetStore.expandedPages.has(child.id)" :page-id="child.id"
                :depth="depth + 1" />
        </div>
    </div>
</template>
