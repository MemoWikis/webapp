...
test.describe('User Registration', () => {
    test('registration page loads correctly', async ({ page }) => {
        ...
        await expect(page.locator('h1.register-title')).toBeVisible({
            timeout: 10000,
        })
    ...
    test('registration with valid data succeeds', async ({ page }) => {
        ...
        const registerButton = page.locator(
            'fieldset button.btn-primary.col-sm-12',
        )
        ...
        try {
            ...
            if (errorMessage) {
                throw new Error(
                    `Registration failed with error: ${errorMessage}`,
                )
            }
            throw error
        }
    ...
    test('registration with empty fields shows validation', async ({
        page,
    }) => {
        // Navigate to registration page
        await page.goto('/Register')
    ...
    test('can navigate to login from registration page', async ({ page }) => {
        ...
        await expect(page.locator('.login-inputs').first()).toBeVisible({
            timeout: 10000,
        })
