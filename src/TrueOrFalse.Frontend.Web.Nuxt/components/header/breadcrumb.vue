...
const updateBreadcrumb = _.throttle(async () => {
  if (breadcrumbEl.value != null && breadcrumbEl.value.clientHeight != null) {
    const width =
      props.headerContainer.clientWidth - props.headerExtras.clientWidth - 30;

    if (width > 0) breadcrumbWidth.value = `width: ${width}px`;
...
function setPageType() {
  if (route.params.topic != null) pageType.value = PageType.Topic;
  else if (route.params.question != null) pageType.value = PageType.Question;
...
async function getBreadcrumb() {
  ...
  var currentWikiId = 0;
  if (!isNaN(sessionWikiId)) currentWikiId = sessionWikiId;
  ...
  if (pageType.value == PageType.Topic) {
    const result = await $fetch<Breadcrumb>(
      `/apiVue/Breadcrumb/GetBreadcrumb/`,
      {
        method: "POST",
        body: data,
    ...
    const result = await $fetch<BreadcrumbItem>(
      `/apiVue/Breadcrumb/GetPersonalWiki/`,
      {
        method: "POST",
        body: data,
        // baseURL: process.client ? config.public.clientBase : config.public.serverBase,
...
<template>
  <div
    v-if="breadcrumb != null && pageType == PageType.Topic"
    id="BreadCrumb"
    ref="breadcrumbEl"
  ...
  >
    <NuxtLink
      :to="`/${encodeURI(breadcrumb.personalWiki.Name.replaceAll(' ', '-'))}/${
        breadcrumb.personalWiki.Id
      }`"
      class="breadcrumb-item"
      v-tooltip="breadcrumb.personalWiki.Name"
      v-if="breadcrumb.personalWiki"
    >
      <font-awesome-icon icon="fa-solid fa-house" />
    </NuxtLink>
    <div
      v-if="
        breadcrumb.rootTopic &&
        breadcrumb.currentTopic &&
        breadcrumb.rootTopic.Id != breadcrumb.currentTopic.Id &&
        breadcrumb.isInPersonalWiki
      "
    >
    ...
    <template
      v-else-if="
        breadcrumb.rootTopic &&
        breadcrumb.personalWiki &&
        breadcrumb.rootTopic.Id != breadcrumb.personalWiki.Id &&
        !breadcrumb.isInPersonalWiki
      "
    >
      ...
      <template v-if="topicStore.id != breadcrumb.rootTopic.Id">
        <NuxtLink
          :to="`/${encodeURI(breadcrumb.rootTopic.Name.replaceAll(' ', '-'))}/${
            breadcrumb.rootTopic.Id
          }`"
          class="breadcrumb-item"
          v-tooltip="breadcrumb.rootTopic.Name"
        >
      ...
      <template #popper>
        <ul>
          <li v-for="s in stackedBreadcrumbItems">
            <NuxtLink
              :to="`/${encodeURI(s.Name.replaceAll(' ', '-'))}/${s.Id}`"
              v-tooltip="s.Name"
            >
              {{ s.Name }}
            </NuxtLink>
    ...
    <template v-for="b in breadcrumbItems">
      <NuxtLink
        :to="`/${encodeURI(b.Name.replaceAll(' ', '-'))}/${b.Id}`"
        class="breadcrumb-item"
        v-tooltip="b.Name"
      >
        {{ b.Name }}
  ...
  <div
    v-else-if="personalWiki != null"
    id="BreadCrumb"
    :style="breadcrumbWidth"
  >
    <NuxtLink
      :to="`/${encodeURI(personalWiki.Name.replaceAll(' ', '-'))}/${
        personalWiki.Id
      }`"
      class="breadcrumb-item"
    ...
    <div
      ...
      v-tooltip="topicStore.name"
      v-if="pageType == PageType.Topic"
    >
      {{ topicStore.name }}
    </div>
