    ...
    return {
      ...
      addImage: () => () => {
        ...
        fileHolder.addEventListener("change", (e: Event) => {
          if (view.state.selection.$from.parent.inlineContent && (<HTMLInputElement>e.target)?.files?.length) {
            startImageUpload(view, (<HTMLInputElement>e.target)?.files![0], schema, uploadFn)
          }
          view.focus()
        })
        fileHolder.click()
      ...
      addBase64Image: (base64String: string) => () => {
        resizeAndUploadBase64Image(base64String, this.editor.view, this.editor.schema, this.options.uploadFn)
        return true
      },
    }
  },
...
function startImageUpload(view: EditorView, file: File, schema: Schema, uploadFn: UploadFn) {
