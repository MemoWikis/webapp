...
export class GoogleMemuchoUser {
    ...
    static async Exists(googleId: string): Promise<boolean> {
        ...
        var doesExist = await $fetch<boolean>('/apiVue/GoogleUsers/UserExists', {
            method: 'POST', body: { googleId: googleId }, credentials: 'include', cache: 'no-cache'
    ...
    static async Login(googleId: string, googleIdToken: string) {
        ...
        await $fetch<boolean>('/apiVue/GoogleUsers/Login', {
            method: 'POST', body: { googleId: googleId, googleToken: googleIdToken }, mode: 'cors', credentials: 'include', cache: 'no-cache'
        }).catch((error) => console.log(error.data))
    }
