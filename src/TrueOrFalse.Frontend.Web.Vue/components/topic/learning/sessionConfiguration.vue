    ...
    <div class="session-configurator col-xs-12 ">
        <div class="session-config-header">
            <div class="filter-button selectable-item session-title" @click="showFilterDropdown = !showFilterDropdown"
                :class="[showFilterDropdown ? 'open' : 'closed', sessionConfigurationStore.activeCustomSettings ? 'activeCustomSettings' : '']">
                Filter
                <div>
        ...
        <div v-show="showFilterDropdown" class="session-config-dropdown row">
            <div class="dropdown-container col-xs-12 col-sm-6" v-click-outside="closeQuestionFilterDropdown"
                ...
                <div class="question-filter-options-button selectable-item"
                    ...
                    <div v-else-if="selectedQuestionFilterOptionsDisplay.length == 0" class="button-placeholder">Wähle
                    ...
                    <div v-else class="question-filter-options-icon-container">
                        ...
                        <div class="icon-counter" v-if="selectedQuestionFilterOptionsExtraCount >= 2">
                            +{{ selectedQuestionFilterOptionsExtraCount }}</div>
                    </div>
                ...
                <div v-if="showQuestionFilterOptionsDropdown" class="question-filter-options-dropdown">
                    <div @click="selectAllQuestionFilter()" class="selectable-item dropdown-item"
                        :class="{ 'item-disabled': !isLoggedIn }">
                        <i class="fas fa-check-square session-select active"
                            v-if="allQuestionFilterOptionsAreSelected"></i>
                    ...
                    <div v-for="q in questionFilterOptions" @click="selectQuestionFilter(q)"
            ...
            <div class="col-xs-12 col-sm-6 question-counter-container">
                ...
                <div class="question-counter"
                    :class="{ 'input-is-active': questionCountInputFocused, 'input-error': questionCountIsInvalid && userHasChangedMaxCount }">
                    <input type="number" min="0" v-model="selectedQuestionCount"
                        v-on:input="setSelectedQuestionCount($event)" @focus="questionCountInputFocused = true"
                        @blur="questionCountInputFocused = false" />
                ...
                <div v-if="questionCountIsInvalid && userHasChangedMaxCount" class="input-error-label">Wähle mindestens
                    1 Frage</div>
            ...
            <div class="dropdown-container col-xs-12 col-sm-6" v-click-outside="closeKnowledgeSummaryDropdown"
                ...
                <div class="knowledge-summary-button selectable-item"
                    ...
                    <div class="knowledge-summary-chip-container">
                        <template v-for="s in sessionConfigurationStore.knowledgeSummary">
                ...
                <div v-if="showKnowledgeSummaryDropdown" class="knowledge-summary-dropdown">
                    <div class="selectable-item dropdown-item" @click="selectAllKnowledgeSummary()"
                        :class="{ 'item-disabled': !isLoggedIn }">
                        <i class="fas fa-check-square session-select active"
                            v-if="allKnowledgeSummaryOptionsAreSelected"></i>
                        <i class="far fa-square session-select" v-else></i>
                        <div class="selectable-item" :class="{ 'item-disabled': !isLoggedIn }">Alles auswählen</div>
                    </div>
                    <div class="dropdown-divider"></div>
                    <div v-for="k in knowledgeSummary" class="dropdown-item selectable-item"
                        :class="{ 'item-disabled': !isLoggedIn }" @click="selectKnowledgeSummary(k)">
            ...
            <div class="dropdown-container col-xs-12 col-sm-6" v-click-outside="closeModeSelectionDropdown"
                ...
                <div v-if="showModeSelectionDropdown" class="mode-change-dropdown">
                    <div>
                        <div class="mode-group-container" @click="selectPracticeMode"
                            :class="{ 'selectable-item': !isPracticeMode, 'no-pointer': isPracticeMode }">
                        ...
                        <div v-if="isPracticeMode" class="mode-group-container">
                            ...
                            <div class="mode-item-container selectable-item"
                                @click="selectPracticeOption('questionOrder', 0)">
                                <div class="mode-sub-label">
                                    Einfache Fragen zuerst
                                </div>
                                <i class="fas fa-dot-circle session-mini-select active"
                                    v-if="practiceOptions.questionOrder == 0"></i>
                                <i class="far fa fa-circle-o session-mini-select" v-else></i>
                            </div>
                            <div class="mode-item-container selectable-item"
                                @click="selectPracticeOption('questionOrder', 1)">
                                <div class="mode-sub-label">
                                    Schwere Fragen zuerst
                                </div>
                                <i class="fas fa-dot-circle session-mini-select active"
                                    v-if="practiceOptions.questionOrder == 1"></i>
                                <i class="far fa fa-circle-o session-mini-select" v-else></i>
                            </div>
                            <div class="mode-item-container selectable-item"
                                @click="selectPracticeOption('questionOrder', 2)"
                                :class="{ 'item-disabled': !isLoggedIn }">
                                <div class="mode-sub-label">
                                    Nicht gewusste Fragen zuerst
                                </div>
                                <i class="fas fa-dot-circle session-mini-select active"
                                    v-if="practiceOptions.questionOrder == 2"></i>
                                <i class="far fa fa-circle-o session-mini-select" v-else></i>
                            </div>
                            <div class="mode-item-container selectable-item"
                                @click="selectPracticeOption('questionOrder', 3)">
                                <div class="mode-sub-label">
                                    Zufällige Fragen
                                </div>
                                <i class="fas fa-dot-circle session-mini-select active"
                                    v-if="practiceOptions.questionOrder == 3"></i>
                                <i class="far fa fa-circle-o session-mini-select" v-else></i>
                            </div>
                            ...
                            <div class="mode-item-container selectable-item"
                                @click="selectPracticeOption('repetition', 0)">
                                <div class="mode-sub-label">
                                    Keine Wiederholung
                                </div>
                                <i class="fas fa-dot-circle session-mini-select active"
                                    v-if="practiceOptions.repetition == 0"></i>
                                <i class="far fa fa-circle-o session-mini-select" v-else></i>
                            </div>
                            <div class="mode-item-container selectable-item"
                                @click="selectPracticeOption('repetition', 1)">
                                <div class="mode-sub-label">
                                    Falsch beantwortete Fragen wiederholen
                                </div>
                                <i class="fas fa-dot-circle session-mini-select active"
                                    v-if="practiceOptions.repetition == 1"></i>
                                <i class="far fa fa-circle-o session-mini-select" v-else></i>
                            </div>
                            <div class="mode-item-container item-disabled selectable-item">
                                ...
                                <i class="fas fa-dot-circle session-mini-select active"
                                    v-if="practiceOptions.repetition == 2"></i>
                                <i class="far fa fa-circle-o session-mini-select" v-else></i>
                            </div>
                    ...
                    <div>
                        <div class="mode-group-container" @click="selectTestMode"
                            :class="{ 'selectable-item': !isTestMode, 'no-pointer': isTestMode }">
                        ...
                        <div v-if="isTestMode" class="mode-group-container">
                            <div class="mode-item-container  selectable-item"
                                @click="selectTestOption('questionOrder', 3)">
                                <div class="mode-sub-label">
                                    Zufällige Fragen
                                </div>
                                <i class="fas fa-dot-circle session-mini-select active"
                                    v-if="testOptions.questionOrder == 3"></i>
                                <i class="far fa fa-circle-o session-mini-select" v-else></i>
                            </div>
                            <div class="mode-item-container  selectable-item"
                                @click="selectTestOption('questionOrder', 0)">
                                <div class="mode-sub-label">
                                    Einfache Fragen zuerst
                                </div>
                                <i class="fas fa-dot-circle session-mini-select active"
                                    v-if="testOptions.questionOrder == 0"></i>
                                <i class="far fa fa-circle-o session-mini-select" v-else></i>
                            </div>
                            <div class="mode-item-container  selectable-item"
                                @click="selectTestOption('questionOrder', 1)">
                                <div class="mode-sub-label">
                                    Schwere Fragen zuerst
                                </div>
                                <i class="fas fa-dot-circle session-mini-select active"
                                    v-if="testOptions.questionOrder == 1"></i>
                                <i class="far fa fa-circle-o session-mini-select" v-else></i>
                            </div>
                            <div class="mode-item-container  selectable-item"
                                @click="selectTestOption('questionOrder', 2)">
                                <div class="mode-sub-label">
                                    Nicht gewusste Fragen zuerst
                                </div>
                                <i class="fas fa-dot-circle session-mini-select active"
                                    v-if="testOptions.questionOrder == 2"></i>
                                <i class="far fa fa-circle-o session-mini-select" v-else></i>
                            </div>
                            <div class="dropdown-spacer"></div>
                        </div>
