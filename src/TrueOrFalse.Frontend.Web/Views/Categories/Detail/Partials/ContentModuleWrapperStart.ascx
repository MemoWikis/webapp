...
<content-module inline-template orig-markdown="<%: Model.Markdown %>" content-module-type="<%: Model.Type %>">
    <li class="module" v-if="!isDeleted" :markdown="markdown" :id="id" :role="button" :data-toggle="modal" :data-target="modalType" :data-component-id="id" :data-markdown="origMarkdown" @click.self="isListening = true" v-cloak>
