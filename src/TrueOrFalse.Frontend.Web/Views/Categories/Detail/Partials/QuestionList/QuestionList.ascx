            ...
            <div @click="expandQuestion(q.Id)">
                ...
                <div class="questionContainer" v-show="showFullQuestion">
                    ...
                    <div class="questionBody">
                        <div class="answer">{{extendedQuestion[q.Id].answer}}</div>
                        <div class="extendedAnswer">{{extendedQuestion[q.Id].extendedAnswer}}</div>
                        <div class="notes">
                            <div class="relatedCategories"><a v-for="c in extendedQuestion[q.Id].categories" :href="c.url">{{c.name}}</a></div>
                            <div class="author"><a :href="extendedQuestion[question.Id].author.url">{{extendedQuestion[q.Id].author.name}}</a></div>
                            <div class="sources"><a v-for="s in extendedQuestion[q.Id].sources" :href="s.url">{{s.name}}</a></div>
                        </div>
                        <div class="questionDetails" :data-question-id="question.Id"></div>
                        <div class="questionFooterIcons">
                            <span>{{extendedQuestion[q.Id].commentCount}}</span>
