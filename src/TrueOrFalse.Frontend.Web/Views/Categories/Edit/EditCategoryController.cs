...
public class EditCategoryController : BaseController
{
    ...
    public EditCategoryController(CategoryRepository categoryRepository, SessionUser sessionUser) : base(sessionUser)
    {
    ...
    public JsonResult AddChild(int childCategoryId, int parentCategoryId, int parentCategoryIdToRemove = -1, bool redirectToParent = false, bool addIdToWikiHistory = false)
    {
        ...
        if (addIdToWikiHistory)
            RecentlyUsedRelationTargets.Add(_sessionUser.UserId, parentCategoryId);
    ...
    {
        var category = new Category(name) { Creator = Sl.UserRepo.GetById(_sessionUser.UserId) };
        category.Visibility = CategoryVisibility.Owner;
        ...
        if (Request["ImageIsNew"] == "true")
        {
            if (Request["ImageSource"] == "wikimedia")
            {
                Resolve<ImageStore>().RunWikimedia<CategoryImageSettings>(
                    Request["ImageWikiFileName"], categoryId, ImageType.Category, _sessionUser.UserId);
            }
            if (Request["ImageSource"] == "upload")
            {
                Resolve<ImageStore>().RunUploaded<CategoryImageSettings>(
                    _sessionUiData.TmpImagesStore.ByGuid(Request["ImageGuid"]), categoryId, _sessionUser.UserId, Request["ImageLicenseOwner"]);
            }
        }
    }
    ...
    {
        if (source == "wikimedia")
            Resolve<ImageStore>().RunWikimedia<CategoryImageSettings>(wikiFileName, categoryId, ImageType.Category, _sessionUser.UserId);
        if (source == "upload")
            Resolve<ImageStore>().RunUploaded<CategoryImageSettings>(
                _sessionUiData.TmpImagesStore.ByGuid(guid), categoryId, _sessionUser.UserId, licenseOwner);
    }
