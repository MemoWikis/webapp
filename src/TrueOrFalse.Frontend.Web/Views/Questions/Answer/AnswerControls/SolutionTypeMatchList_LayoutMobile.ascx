...
<script type="text/javascript">
   ...
   int i = 0;
    ...
    $('#matchlist-mobilepairs').append($('<div class="matchlist-mobilepairrow col-sm-12 row form-group" id="matchlist-mobilepairrow-<%= i %>">')
        ...
        .append($('<div class="col-sm-12">').append($('<select class="form-control" name="matchlist-select-<%= i %>">').append($('<option>Keine Zuordnung</option>'))))
    );
    <% i++;
    }
    ...
    { %>
    $('select[name*="matchlist-select-"]').each(function (index, element) {
