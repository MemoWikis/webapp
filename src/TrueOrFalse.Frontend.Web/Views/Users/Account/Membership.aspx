...
<%if (Model.IsMember) {%>
    ...
    <h3>Danke, dass du uns unterstützt!</h3>

    <h4>Deine Mitgliedschaft:</h4>
    
    <p>Monatlicher Preis: <%= String.Format("{0:C}", Model.Membership.PricePerMonth) %></p>

    <p>Preiskategorie: <%= String.Format("{0:d}", Model.Membership.PriceCategory.ToString()) %></p>

    <p>Gültig bis einschließlich: <%= String.Format("{0:d}", Model.Membership.PeriodEnd) %></p>
