...
<p style="padding-left: 15px; margin-bottom: 0px;">
    <% foreach (var category in Model.Categories) {%>
            <p style="margin-bottom: 0px; padding-left: 15px; padding-bottom: 5px; line-height: 12px;">
                ...
                <% if (category.CountQuestions == 0) {%> (0 Fragen)
                <% } else if (category.CountQuestions == 1) {%> <a href="<%: Links.QuestionWithCategoryFilter(Url, category) %>">(1 Frage)</a>
                <% } else {%><a href="<%: Links.QuestionWithCategoryFilter(Url, category) %>">(<%: category.CountQuestions %> Fragen)</a>
                <%} %>
            </p>
    <%} %>
</p>
