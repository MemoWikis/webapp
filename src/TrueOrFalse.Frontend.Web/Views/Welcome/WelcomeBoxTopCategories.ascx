...
<% foreach (var category in Model.Categories) {%>
    <div class="LabelItem LabelItem-Category">
        ...
        <% if (category.GetCountQuestions() == 0) { %> 
            (0 Fragen)
        <% } else if (category.GetCountQuestions() == 1) { %> 
            <a href="<%: Links.QuestionWithCategoryFilter(Url, category) %>" class="NumberQuestions">(1 Frage)</a>
        <% } else {%>
            <a href="<%: Links.QuestionWithCategoryFilter(Url, category) %>">(<%: category.GetCountQuestions() %> Fragen)</a>
        <%} %>
