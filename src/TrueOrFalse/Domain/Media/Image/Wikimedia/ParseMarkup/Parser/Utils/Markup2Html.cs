...
namespace TrueOrFalse.WikiMarkup
{
    public class Markup2Html
    {
        ...
        public static string TransformLinkMarkup(string markup)
        {
            ...
            foreach (var languageToken in WikiLanguage.GetAllLanguages().Select(x => x.LanguageToken))
            {
                markup = Regex.Replace(markup, "\\[\\[:?" + languageToken + ":(.*?)\\|(.*?)\\]\\]",
                    match => (languageToken == "de" || languageToken == "en") ? 
                                match.Groups[2].Value + " (http://" + languageToken + ".wikipedia.org/wiki/" + match.Groups[1].Value.Replace(' ', '_') + ")" :
                                match.Groups[2].Value);
            }
