...
public class UserRepo : RepositoryDbBase<User>
{
    ...
    {   // Delete
        ...
        //Session.CreateSQLQuery("DELETE FROM answer WHERE UserId = :userId").SetParameter("userId", userId).ExecuteUpdate();
        Session.CreateSQLQuery("delete q.*, g.*, p.*, w.* From questionview q left join game_round g ON q.Round_id = g.Id left join game_player p ON q.Player_Id = p.Id left join widgetView w ON q.WidgetView_id = w.Id Where q.UserId = :userid; ").SetParameter("userid", userId ).ExecuteUpdate();
